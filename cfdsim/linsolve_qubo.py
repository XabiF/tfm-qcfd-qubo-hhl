import cfd
import cfdplot
from .quboutil import prepare_qubo_params, process_qubo_params, qubo_solve

def syslin_solve(sim: cfd.Simulation, n, A, b, var_kind):
    chimeraplot = sim.param["qubo-chimera-plot"]
    qubo_encoding = sim.param["qubo-encoding"]

    # QUBO cost function: ||A*x - b||^2
    arr_real, const_coefs = prepare_qubo_params(sim, n, var_kind, qubo_encoding)
    objective = sum((sum(A[i,j]*arr_real[j] for j in range(n)) - b[i])**2 for i in range(n))

    solution = qubo_solve(sim, objective)
    arr_sol, bit_tensor = process_qubo_params(sim, n, const_coefs, solution)

    if chimeraplot:
        R = sim.param["R"]
        cfdplot.plot_qubo_chimera(n, R, bit_tensor)

    return arr_sol
